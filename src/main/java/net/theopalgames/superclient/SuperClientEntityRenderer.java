...
public class SuperClientEntityRenderer extends EntityRenderer implements IResourceManagerReloadListener
{
	static double camX = 0;
	static double camY = 0;
	static double camZ = 0;
	static boolean camLocked = false;
    ...
    public static float maximumThirdPersonDistance = SuperClientConfig.cam.thirdPersonDistance;

    public static float thirdPersonDistance = maximumThirdPersonDistance;

    //private final float thirdPersonDistance = maximumThirdPersonDistance;
    /** Previous third person distance */
    private float thirdPersonDistancePrev = thirdPersonDistance;
    /** Smooth cam yaw */
    ...
    public SuperClientEntityRenderer(Minecraft mcIn, IResourceManager resourceManagerIn)
    {
    ...
    @Override public void switchUseShader()
    {
     ...
     */
    @Override public void loadEntityShader(@Nullable Entity entityIn)
    {
    ...
    @Override public void loadShader(ResourceLocation resourceLocationIn)
    {
        try
        {
	...
	@Override public void updateRenderer()
    {
        ...
        this.thirdPersonDistance = this.maximumThirdPersonDistance;
        if (mc.gameSettings.thirdPersonView == 2)
        	this.thirdPersonDistance = -Math.abs(thirdPersonDistance);
        this.thirdPersonDistancePrev = thirdPersonDistance;
    ...
    @Override public ShaderGroup getShaderGroup()
    {
        return this.shaderGroup;
    }

    @Override public void updateShaderGroupSize(int width, int height)
    {
        if (OpenGlHelper.shadersSupported)
        {
            if (this.shaderGroup != null)
     ...
     */
    @Override public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null)
        {
            if (this.mc.world != null)
            {
                ...
                {
                    @Override public boolean apply(@Nullable Entity p_apply_1_)
                    {
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        {
            double d3 = (double)(this.thirdPersonDistancePrev + (thirdPersonDistance - this.thirdPersonDistancePrev) * partialTicks);
            ...
            {
            	float f7 = entity.rotationYaw; 
                float f8 = entity.rotationPitch;
                float f1; 
                float f2;
                f1 = (float) (entity.rotationYaw + SuperClientEvent.yaw);
                f2 = (float) (entity.rotationPitch + SuperClientEvent.pitch);
                if (camLocked)
                {
                	f1 = (float) camX;
                	f2 = (float) camY;
                ...
                GlStateManager.rotate(entity.rotationPitch - f8, 1.0F, 0.0F, 0.0F);
                GlStateManager.rotate(entity.rotationYaw - f7, 0.0F, 1.0F, 0.0F);
                GlStateManager.translate(0.0F, 0.0F, (float)(-d3));
                GlStateManager.rotate(f7 - entity.rotationYaw, 0.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f8 - entity.rotationPitch, 1.0F, 0.0F, 0.0F);
            }
        }
        else
        {
        ...
        if (!this.mc.gameSettings.debugCamEnable)
        {
            ...
            net.minecraftforge.client.event.EntityViewRenderEvent.CameraSetup event = new net.minecraftforge.client.event.EntityViewRenderEvent.CameraSetup(this, entity, state, partialTicks, yaw + (int)SuperClientEvent.yaw, pitch + (int)SuperClientEvent.pitch, roll + (int)SuperClientEvent.roll);
    ...
    @Override public void disableLightmap()
    {
    ...
    @Override public void enableLightmap()
    {
        GlStateManager.setActiveTexture(OpenGlHelper.lightmapTexUnit);
            ...
            if (this.mc.currentScreen != null)
            {
                ...
                catch (Throwable throwable)
                {
                    ...
                    crashreportcategory.addDetail("Screen name", new ICrashReportDetail<String>()
                    {
                        ...
                        {
                            return SuperClientEntityRenderer.this.mc.currentScreen.getClass().getCanonicalName();
                        }
                    });
                    ...
                    crashreportcategory.addDetail("Screen size", new ICrashReportDetail<String>()
                    {
                        @Override public String call() throws Exception
                        {
                            return String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(), SuperClientEntityRenderer.this.mc.displayWidth, SuperClientEntityRenderer.this.mc.displayHeight, scaledresolution.getScaleFactor());
                        }
                    });
    ...
    {
        if (Integer.parseInt(mc.renderGlobal.getDebugInfoRenders().substring(3).split("/")[0]) > 10 && this.mc.renderGlobal.hasNoChunkUpdates() && !this.mc.getIntegratedServer().isWorldIconSet())
        {
    ...
    @Override public void renderStreamIndicator(float partialTicks)
    {
        this.setupOverlayRendering();
    }
     ...
     */
    @Override public void setupOverlayRendering()
    {
    ...
    private void setupFog(int startCoords, float partialTicks)
    {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPotionActive(MobEffects.BLINDNESS))
        {
        	float fog = (float)SuperClientConfig.cam.blindness;
            float f1 = fog;
            int i = ((EntityLivingBase)entity).getActivePotionEffect(MobEffects.BLINDNESS).getDuration();

            if (i < 20)
            {
                f1 = fog + (this.farPlaneDistance - fog) * (1.0F - (float)i / 20.0F);
            }
    ...
    @Override public void setupFogColor(boolean black)
    {
        if (black)
        {
    ...
    @Override public MapItemRenderer getMapItemRenderer()
    {
    ...
    @Override public void displayItemActivation(ItemStack p_190565_1_)
    {
